...
def save_banner_type(request):
    if request.method == 'POST':
        try:
            ...
            # Fetch the shop's access token from the database
            try:
                shop = Shop.objects.get(shop_url=shop_url)
                access_token = shop.access_token  # Assuming the Shop model has an access_token field
                print(f"Access token for {shop_url}: {access_token}")  # Debugging line to check access token
            except Shop.DoesNotExist:
                return JsonResponse({'error': 'Shop not found in the database'}, status=404)

            # Check if the Store already has a banner_type
            store, created = Store.objects.get_or_create(shop_url=shop_url)
            
            # If store exists, update the banner_type
            store.banner_type = banner_type
            store.save()  # Save the updated Store instance
            print(f"Store banner_type updated to: {banner_type}")  # Debugging line for store update

            # GraphQL mutation for updating the banner type in Shopify
            mutation = """
            ...
            print(f"GraphQL response: {response.status_code}, {response.text}")  # Debugging line for response

            if response.status_code != 200:
                return JsonResponse({'error': 'Failed to update banner type on Shopify'}, status=500)
...
def save_selected_countries(request):
    if request.method == 'POST':
        try:
            ...
            # Check if the Store already has selected countries
            store, created = Store.objects.get_or_create(shop_url=shop_url)
            
            # If store exists, update the selected_countries
            store.selected_countries = selected_countries
            store.save()  # Save the updated Store instance
            print(f"Store selected_countries updated to: {selected_countries}")  # Debugging line for store update

            # GraphQL mutation for updating the selected countries in Shopify
            mutation = """
            ...
            if response.status_code != 200:
                return JsonResponse({'error': 'Failed to update selected countries on Shopify'}, status=500)
    ...
    if request.method == 'POST':
        ...
        charge_data = {
            "recurring_application_charge": {
                ...
                "return_url": f"https://5b6c-103-105-234-92.ngrok-free.app/shopify_app/accept_charge?shop_url={shop_url}&access_token={access_token}",
                "trial_days": 7,
                "test": True,
            }
        }
...
def accept_charge(request):
    if request.method == 'GET':
        ...
        if response.status_code == 200:
            ...
            charge_data = response.json().get("recurring_application_charge", {})
            plan_price = charge_data.get("price")

            # Map plan_price to plan_name
            plan_name_mapping = {
                "1.00": "Basic",
                "2.00": "Standard",
                "3.00": "Advanced",
            }
            plan_name = plan_name_mapping.get(str(plan_price), "Unknown Plan")

            # Save or update the Plan model
            ...
            # Redirect after updating or creating the plan
            return HttpResponseRedirect(f"https://{shop_url}/admin/apps/4d48baea68b47ab91304b6080392c538")
...
def cookie_save_preferences(request):
    if request.method == 'POST':
        try:
            ...
            if not shop_url or not preference or not ip_address:
                return JsonResponse({'status': 'error', 'message': 'Missing required fields.'}, status=400)

            # Check if the ip_address already exists in the database
            existing_record = CookiePreference.objects.filter(ip_address=ip_address).first()

            if existing_record:
                # Update the existing record
                existing_record.shop_url = shop_url
                existing_record.preference = preference
                existing_record.url = url
                existing_record.created_date = created_date
                existing_record.selected_preferences = selected_preferences
                existing_record.save()

                return JsonResponse({'status': 'success', 'message': 'Preferences updated successfully.'}, status=200)
            else:
                # If ip_address is unique, create a new record
                obj = CookiePreference.objects.create(
                    shop_url=shop_url,
