...
function Checklist() {
  ...
  useEffect(() => {
    const fetchPlan = async () => {
      try {
        ...
        setPlan(data.plan);
        setIsBasicPlan(data.plan === 'Basic'); // Update based on your plan structure
  ...
  const handlePlanSelect = async (plan) => {
    ...
    try {
      ...
      const chargeResponse = await fetch(
        "https://5b6c-103-105-234-92.ngrok-free.app/shopify_app/create_charge/",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    ...
    >
      ...
      <div style={{ cursor: 'pointer' }}>
        <LegacyCard sectioned>
          <p style={{ fontWeight: '650', marginBottom: '7px', paddingTop: '5px' }}>Show re-open banner</p>
          <p>
