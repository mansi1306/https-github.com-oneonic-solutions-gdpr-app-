    ...
    <div style={{width:'950px',marginLeft:'-160px'}}><Page>
      ...
      {modalActive && (
        <Modal open={modalActive} onClose={toggleModal} title="Edit regions">
                  ...
                  <div key={region} style={{ marginBottom: '10px' }}>
                    ...
                    {expandedRegions[region] && (
                      <div style={{ marginLeft: '20px', marginTop: '5px'}}>
                        <div style={{ display: 'block' }}> {/* Block layout for vertical list */}
                          {regionsData[region].map((country, index) => (
                            <div key={index} style={{ marginBottom: '5px' }}>
                              <Checkbox
                                label={typeof country === 'string' ? country : country.name}
                                checked={regions[region]} // Adjust this according to your specific country selection logic
                                onChange={() => handleCountryChange(region, typeof country === 'string' ? country : country.name)}
                              />
                              {/* Optional: Show "Recommended" label */}
                              {typeof country === 'object' && country.recommended && (
                                <div style={{marginLeft:'430px',marginTop:'-22px'}}><Badge className="recommended-banner">Recommended</Badge></div>
                              )}
                            </div>
                          ))}
                        </div>
          ...
          <Modal.Section>
            <div style={{marginLeft:'450px'}}><ButtonGroup>
