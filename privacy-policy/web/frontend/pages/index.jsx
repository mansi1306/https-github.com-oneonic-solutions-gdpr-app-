...
function CountryForm() {
  ...
  const handleRegionChange = (region) => {
    setRegions((prevRegions) => {
      ...
      if (allCountries && !allChecked) {
        setAllCountries(false);
      }

      Object.keys(updatedRegion).forEach((country) => {
        updatedRegion[country] = !updatedRegion[country];
      });
    ...
    const initialRegions = Object.keys(regionsData).reduce((acc, region) => {
      ...
      regionsData[region].forEach(item => {
        if (typeof item === 'string') {
          acc[region][item] = true;
        } else {
          acc[region][item.name] = true;
        }
      });
      return acc;
    }, {});
    ...
    Object.keys(regionsData).forEach((region) => {
      if (regions[region]) {
        regionsData[region].forEach((country) => {
          ...
          const countryName = isString ? country : country.name;
          const isRecommended = !isString && country.recommended;

          if (isRecommended || regions[region][countryName]) {
            count++;
          }
        });
      }
                            ...
                            return (
                              ...
                              >
                                <Checkbox
                                  ...
                                  checked={isRecommended || isChecked}
