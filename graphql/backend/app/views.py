    ...
    if request.method == 'POST':
        try:
            ...
            customer_data = {
                ...
                "orders": orders_requested  # Include order data if available
            }

            # Logic to upload data to Shopify (URL provided in payload)
            if 'data_request' in payload:
                upload_url = payload['data_request']['url']
                response = requests.put(upload_url, json=customer_data)
                if response.status_code != 200:
                    return JsonResponse({"error": "Failed to upload data to Shopify"}, status=500)
...
def save_banner_type(request):
    if request.method == 'POST':
        try:
            ...
            # GraphQL mutation for updating banner type
            mutation = """
            mutation($shopUrl: String!, $bannerType: String!) {
                updateStoreBanner(shopUrl: $shopUrl, bannerType: $bannerType) {
                    shopUrl
                    bannerType
                }
            }
            ...
            # Assuming the access token is available
            access_token = "your_shopify_access_token"  # You should fetch this dynamically
            response = requests.post(
                f"https://{shop_url}/admin/api/2025-01/graphql.json",
                headers={
                    **headers,
                    'X-Shopify-Access-Token': access_token,
                },
            ...
            if response.status_code != 200:
                return JsonResponse({'error': 'Failed to update banner type'}, status=500)
...
def save_selected_countries(request):
    if request.method == 'POST':
        try:
            ...
            if not shop_url or not selected_countries:
                return JsonResponse({'error': 'Shop URL and Selected Countries are required'}, status=400)

            # GraphQL mutation for updating selected countries
            mutation = """
            mutation($shopUrl: String!, $selectedCountries: [String!]!) {
                updateStoreCountries(shopUrl: $shopUrl, selectedCountries: $selectedCountries) {
                    shopUrl
                    selectedCountries
                }
            }
            ...
            if response.status_code != 200:
                return JsonResponse({'error': 'Failed to update selected countries'}, status=500)
...
def save_theme(request):
    if request.method == 'POST':
        try:
            ...
            if not shop_url or not theme_label:
                return JsonResponse({'error': 'Shop URL and Theme Label are required'}, status=400)

            # GraphQL mutation for saving the selected theme
            mutation = """
            mutation($shopUrl: String!, $themeLabel: String!) {
                updateStoreTheme(shopUrl: $shopUrl, themeLabel: $themeLabel) {
                    shopUrl
                    selectedTheme
                }
            }
            ...
            if response.status_code != 200:
                return JsonResponse({'error': 'Failed to save theme'}, status=500)
...
def get_store_data(request):
    # Get the shop URL from the request parameters (e.g., ?shop_url=myshop.myshopify.com)
    ...
    try:
        # Retrieve access token for Shopify (this should come from your store or session)
        access_token = "your_shopify_access_token"  # Replace with actual token
        complete_shop_url = f"https://{shop_url}"

        # Define the GraphQL query to fetch store data (banner_type, selected_countries, selected_theme)
        query = """
        query($shopUrl: String!) {
        ...
        if not store_data:
            return JsonResponse({'error': 'Store data not found'}, status=404)

        # Return the store data (bannerType, selectedCountries, selectedTheme)
    ...
    if request.method == 'POST':
        try:
            ...
            if plan_name and shop_url:
                # Define the GraphQL mutation to update or create a plan for the shop
                mutation = """
                mutation($shopUrl: String!, $planName: String!) {
                  selectPlan(shopUrl: $shopUrl, planName: $planName) {
                    success
                    message
                  }
                }
                ...
                # Assuming we have an access token (replace with actual token)
                access_token = "your_shopify_access_token"  # Replace with actual token
                complete_shop_url = f"https://{shop_url}"
                ...
                # Send the GraphQL mutation request to Shopify API
                response = requests.post(
                    f"{complete_shop_url}/admin/api/2025-01/graphql.json",  # Replace with actual API version
                    headers=headers,
                    json={'query': mutation, 'variables': variables}
                )

                if response.status_code != 200:
                    return JsonResponse({'error': 'Failed to send request to Shopify'}, status=500)

                # Parse the response from Shopify
                data = response.json()

                # Assuming the response includes a 'selectPlan' key with success or message
                success = data.get('data', {}).get('selectPlan', {}).get('success', False)
                message = data.get('data', {}).get('selectPlan', {}).get('message', 'No message')
...
def get_plan(request):
    if request.method == 'GET':
        ...
        if shop_url:
            try:
                # Define the GraphQL query to fetch the plan for the shop
                query = """
                query($shopUrl: String!) {
                  getPlan(shopUrl: $shopUrl) {
                    planName
                    error
                  }
                }
                ...
                # Send the GraphQL query request to the Shopify API (or your GraphQL endpoint)
                response = requests.post(
                    f"{complete_shop_url}/admin/api/2025-01/graphql.json",  # Replace with actual API version
                    headers=headers,
                    json={'query': query, 'variables': variables}
                ...
                plan_name = data.get('data', {}).get('getPlan', {}).get('planName')
                if plan_name:
                    return JsonResponse({'plan': plan_name}, status=200)
                else:
                    error_message = data.get('data', {}).get('getPlan', {}).get('error', 'Plan not found')
                    return JsonResponse({'plan': None, 'error': error_message}, status=404)
...
def create_recurring_application_charge(request):
    if request.method == 'POST':
        ...
        # GraphQL mutation to create recurring charge
        query = """
        mutation CreateRecurringCharge($name: String!, $price: Float!, $returnUrl: String!, $trialDays: Int!) {
          recurringApplicationChargeCreate(input: {
            name: $name,
            price: $price,
            returnUrl: $returnUrl,
            trialDays: $trialDays,
        ...
        variables = {
            ...
            "returnUrl": "https://yourapp.com/shopify_app/accept_charge",
            "trialDays": 7
        }
        ...
        if response.status_code == 200:
            data = response.json()
            recurring_charge = data.get("data", {}).get("recurringApplicationChargeCreate", {}).get("recurringApplicationCharge")
            if recurring_charge:
                return JsonResponse({"success": True, "charge": recurring_charge})
            else:
                error_message = data.get("data", {}).get("recurringApplicationChargeCreate", {}).get("userErrors", [])
...
def accept_charge(request):
    if request.method == 'GET':
        ...
        # Extract the Bearer token if it exists
        if access_token and access_token.startswith('Bearer '):
            access_token = access_token.split(' ')[1]

        # Define GraphQL mutation to activate the charge
        query = """
        mutation AcceptCharge($chargeId: ID!) {
          recurringApplicationChargeActivate(id: $chargeId) {
            recurringApplicationCharge {
              id
              status
            }
        ...
        if response.status_code == 200:
            ...
            charge_data = data.get("data", {}).get("recurringApplicationChargeActivate", {})
            recurring_charge = charge_data.get("recurringApplicationCharge")

            if recurring_charge:
                return JsonResponse({"success": True, "charge": recurring_charge})
            else:
                # Handle errors if any
                user_errors = charge_data.get("userErrors", [])
                return JsonResponse({"success": False, "errors": user_errors}, status=400)
        else:
            error_message = response.json().get('errors', 'Unknown error')
...
def get_access_token(request):
    if request.method == 'POST':
        ...
        if not shop_url:
            return JsonResponse({'error': 'Missing shop_url'}, status=400)

        try:
            # Define the GraphQL query to fetch the access token for the shop
            query = """
            query($shopUrl: String!) {
              getShopAccessToken(shopUrl: $shopUrl) {
                accessToken
                error
              }
            }
            ...
            # Variables to send in the query
            variables = {
                "shopUrl": shop_url
            }

            # Replace with the actual Shopify access token for the admin panel (not the store)
            admin_access_token = "your_shopify_admin_access_token"  # Replace with actual admin access token
            complete_shop_url = f"https://{shop_url}"

            # Set up the headers for Shopify GraphQL request
            headers = {
                'X-Shopify-Access-Token': admin_access_token,
                'Content-Type': 'application/json',
            }

            # Send the GraphQL query to Shopify API
            response = requests.post(
                f"{complete_shop_url}/admin/api/2025-01/graphql.json",  # Replace with the correct API version
                headers=headers,
                json={'query': query, 'variables': variables}
            )
            ...
            # Parse the response JSON from Shopify
            data = response.json()

            # Extract the access token from the response
            access_token = data.get('data', {}).get('getShopAccessToken', {}).get('accessToken')

            if access_token:
                return JsonResponse({'access_token': access_token}, status=200)
            else:
                error_message = data.get('data', {}).get('getShopAccessToken', {}).get('error', 'Access token not found')
...
def save_shop_preferences(request):
    if request.method == 'POST':
        try:
            data = json.loads(request.body.decode('utf-8'))
            shop_url = data.get('shop_url')
            show_reopen_button = data.get('show_reopen_button')  # Expecting 'true' or 'false' from the frontend

            if not shop_url:
                return JsonResponse({'error': 'Shop URL is required'}, status=400)
            
            # Get or create shop preferences in the database
            preferences, created = ShopPreferences.objects.get_or_create(shop_url=shop_url)
            preferences.show_reopen_button = show_reopen_button == 'true'  # Convert string to boolean
            preferences.save()

            # GraphQL query for saving the selected preferences (Here we're using a regular query)
            query = """
            query($shopUrl: String!, $showReopenButton: Boolean!) {
                updateShopPreferences(shopUrl: $shopUrl, showReopenButton: $showReopenButton) {
                    shopUrl
                    showReopenButton
                }
            }
            ...
            variables = {
                "shopUrl": shop_url,
                "showReopenButton": preferences.show_reopen_button
            }
            ...
            # Assuming the access token is available
            access_token = "your_shopify_access_token"  # Fetch this dynamically if required
            response = requests.post(
                f"https://{shop_url}/admin/api/2025-01/graphql.json",  # Ensure the URL is correct
                headers={
                    **headers,
                    'X-Shopify-Access-Token': access_token,
                },
            ...
            if response.status_code != 200:
                return JsonResponse({'error': 'Failed to save preferences on Shopify'}, status=500)

            # Return the success message with updated data
            return JsonResponse({'message': 'Shop preferences saved successfully', 'show_reopen_button': preferences.show_reopen_button}, status=200)
...
def save_privacy_policy_settings(request):
    if request.method == 'POST':
        try:
            ...
            # Get or create privacy policy settings for the shop
            ...
            # GraphQL query for saving the privacy policy settings (Here we're using a regular query)
            query = """
            query($shopUrl: String!, $showGooglePrivacyPolicy: Boolean!, $showPrivacyPolicyLink: Boolean!, $useSpecificPrivacyPolicy: Boolean!, $selectedOption: String!) {
                updatePrivacyPolicySettings(shopUrl: $shopUrl, showGooglePrivacyPolicy: $showGooglePrivacyPolicy, showPrivacyPolicyLink: $showPrivacyPolicyLink, useSpecificPrivacyPolicy: $useSpecificPrivacyPolicy, selectedOption: $selectedOption) {
                    shopUrl
                    showGooglePrivacyPolicy
                    showPrivacyPolicyLink
                    useSpecificPrivacyPolicy
                    selectedOption
                }
            }
            """
            ...
            variables = {
                "shopUrl": shop_url,
                "showGooglePrivacyPolicy": show_google_privacy_policy,
                "showPrivacyPolicyLink": show_privacy_policy_link,
                "useSpecificPrivacyPolicy": use_specific_privacy_policy,
            ...
            if response.status_code != 200:
                return JsonResponse({'error': 'Failed to save privacy policy settings on Shopify'}, status=500)

            # Return the success message with updated data
            return JsonResponse({'message': 'Privacy policy settings saved successfully'}, status=200)
...
def display_shop_preferences(request):
    ...
    if shop_url:
        try:
            # Fetch the shop preferences from the database
            preference = ShopPreferences.objects.get(shop_url=shop_url)

            # GraphQL query for retrieving the shop preferences
            query = """
            query($shopUrl: String!) {
                getShopPreferences(shopUrl: $shopUrl) {
                    shopUrl
                    showReopenButton
                }
            }
            ...
            if response.status_code != 200:
                return JsonResponse({'error': 'Failed to retrieve shop preferences from Shopify'}, status=500)

            # Parse the response to extract the relevant data
            data = response.json()
            shop_preferences = data.get('data', {}).get('getShopPreferences', {})

            if shop_preferences:
                return JsonResponse({'status': 'success', 'preference': shop_preferences})

            return JsonResponse({'status': 'error', 'message': 'Shop preferences not found'}, status=404)

        except ShopPreferences.DoesNotExist:
            return JsonResponse({'status': 'error', 'message': 'Shop URL not found in local database'}, status=404)
        except Exception as e:
